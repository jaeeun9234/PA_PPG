# -*- coding: utf-8 -*-
# HeartSync BLE Collector — FOI + ln-ratio + smoothing + RT fix
# - BeatDetector window widened (15 samples) to capture physiological rise
# - Foot search widened (first 8 points before peak)
# - Enforce minimum RT >= 100 ms (discard too-short beats)
# - ln-ratio term + AUSPR smoothing/clamp + relaxed TD term
# - auto-baseline maintained
        ...
        if len(self.buffer) == self.buffer.maxlen:
            ...
            if y_mid > y_prev and y_mid > y_next:
                if (i_mid - self.last_peak_idx) >= self.refract and y_mid >= self.min_prom:
                    ...
                    # foot in first 8 samples (0..7) — before mid
                    search_end = min(8, mid)  # safety
...
async def connect_and_run():
    ...
    def parse_and_process(data: bytearray):
        ...
        yL = smoothL.push(xL);     yR = smoothR.push(xR)
            ...
            with open(BEAT_CSV, "a", newline="") as f:
                ...
                for (bL, bR) in pairs:
                    ...
                    if np.isfinite(min_rt) and min_rt < MIN_RT_MS:
                        # discard too-short rise (likely artifact of narrow window or noise)
                        continue
                    ...
                    print(f"[{risk}] beat#{beat_counter} AUSPR={auspr:.3f} ΔTD={dt_ms:.1f}ms RTmin={min_rt:.1f}ms HSI={hsi:.2f} marker={current_marker} α={FOI_ALPHA}")
