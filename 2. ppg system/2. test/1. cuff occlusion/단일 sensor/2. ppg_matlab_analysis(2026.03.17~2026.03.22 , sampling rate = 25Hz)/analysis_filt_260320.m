...
OD_red = T.OD_red;
OD_ir  = T.OD_ir;
OD_red_s = T.OD_red_slide;
OD_ir_s = T.OD_ir_slide;
...
OD_red_base_mean = mean(OD_red(idx_base), 'omitnan');
dOD_red = OD_red - OD_red_base_mean;

OD_ir_base_mean = mean(OD_ir(idx_base), 'omitnan');
dOD_ir = OD_ir - OD_ir_base_mean;
...
figure;
idx = T.spo2_valid == 1;
%plot(t(idx), spo2(idx), "LineWidth", 1.2);
...
%% ===== 8) Plot 4: OD_red, OD_ir (slide)=====
figure;
plot(tt, OD_red_s(mask)); hold on;
plot(tt, OD_ir_s(mask));
xline(T1,'k--');    xline(T2,'k--');    xline(T3,'k--');
grid on;
xlabel("Time (s)");
ylabel("Optical Density (relative)");
...
dA_660 = dOD_red(mask)/ln10;       % 660nm
dA_880 = dOD_ir(mask)/ln10;        % 880nm
...
dA_660 = OD_red_s(mask)/ln10;       % 660nm
dA_880 = OD_ir_s(mask)/ln10;        % 880nm
...
figure;
plot(tt, dHbO2_env, LineWidth = 1.2); hold on;
plot(tt, dHHb_env, LineWidth = 1.2); hold on;
plot(tt, dHbT_env, LineWidth = 1.2);
xline(T1,'k--');    xline(T2,'k--');    xline(T3,'k--');
grid on;
title("HbO_2, HHb, tHb (env) [DC]");
legend("HbO_2(env)", "HHb(env)", "tHb(env)", "Location", "best");
...
%% HbDiff (env)
HbDiff_env = envelope(abs(HbDiff), 100);
figure;
plot(tt, HbDiff_env);
xline(T1,'k--');    xline(T2,'k--');    xline(T3,'k--');
grid on;
...
%% 변화량 plot(DC : envelope, AC : 기본)
figure;
plot(tt, dHbO2_env, "LineWidth", 1.2); hold on; plot(tt, DHbO2); 
xline(T1,'k--');    xline(T2,'k--');    xline(T3,'k--');
title("HbO_2 (env) [DC/AC]"); legend("HbO2 [DC] (env)", "HbO2 [AC]");
grid on;

figure;
plot(tt, dHHb_env, "LineWidth", 1.2); hold on; plot(tt, DHHb); 
xline(T1,'k--');    xline(T2,'k--');    xline(T3,'k--');
title("HHb (env) [DC/AC]"); legend("HHb [DC] (env)", "HHb [AC]");
grid on;

figure;
plot(tt, dHbT_env, "LineWidth", 1.2); hold on; plot(tt, DtHb); 
xline(T1,'k--');    xline(T2,'k--');    xline(T3,'k--');
title("tHb (env) [DC/AC]"); legend("tHb [DC] (env)", "tHb [AC]");
grid on;
