...
cycles = parsePhaseOcclusion(t_trial, phase);
...
subplot(2,1,1);
plot(t_trial, red_raw(mask)); hold on;
plot(t_trial, red_dc(mask), Linewidth = 1.2);
drawPhaseOcclusion(cycles);
grid on;
xlabel("Time (s)");
title("RED: raw & dc & ac");
...
subplot(2,1,1);
plot(t_trial, ir_raw(mask)); hold on;
plot(t_trial, ir_dc(mask), LineWidth=1.2);
drawPhaseOcclusion(cycles);
grid on;
xlabel("Time (s)");
title("IR: raw & dc & ac");
...
figure;
plot(t_trial, OD_red_dc(mask)); hold on;
plot(t_trial, OD_ir_dc(mask));
drawPhaseOcclusion(cycles);
grid on;
xlabel("Time (s)");
...
drawPhaseOcclusion(cycles); grid on;
xlabel("Time (s)");
...
figure;
plot(t_trial, SpO2_cut, 'LineWidth', 1.2); hold on;
plot(t_trial, sto2_ref, 'LineWidth', 1.2);
drawPhaseOcclusion(cycles);
legend('SpO_2', 'StO_2', 'Location', 'best');
grid on;
xlabel('Time (s)');
ylabel('%');
title('SpO_2, StO_2');
...
PI0_IR = mean(PI_IR(t0_idx), 'omitnan');

PI_norm_IR = PI_IR / PI0_IR;
PI_norm_IR = movmean(PI_norm_IR, round(Fs*2), 'omitnan');

figure;
plot(t_trial, PI_norm_IR(mask));
drawPhaseOcclusion(cycles); grid on;
xlabel("Time (s)");
...
PI0_RED = mean(PI_RED(t0_idx), 'omitnan');

PI_norm_RED = PI_RED / PI0_RED;
PI_norm_RED = movmean(PI_norm_RED, round(Fs*2), 'omitnan');

figure;
plot(t_trial, PI_norm_RED(mask));
drawPhaseOcclusion(cycles); grid on;
xlabel("Time (s)");
...
%% 노이즈 확인용 plot
figure; 
subplot(2,2,1); plot(t_trial, ir_dc(mask)); grid on; title("IR DC (masked)");
subplot(2,2,3); plot(t_trial, ir_ac(mask)); grid on; title("IR AC (masked)");
subplot(2,2,2); plot(t_trial, red_dc(mask)); grid on; title("RED DC (masked)");
subplot(2,2,4); plot(t_trial, red_ac(mask)); grid on; title("RED AC (masked)");
...
drawPhaseOcclusion(cycles); title("HbO_2  [DC/AC]"); legend("HbO_2 [DC]", "HbO_2 [AC]", "Location", "best");
ylim([ymin ymax]);
grid on;

figure;
plot(t_trial, dHHb, "LineWidth", 1.2); hold on; plot(t_trial, DHHb); 
drawPhaseOcclusion(cycles); title("HHb [DC/AC]"); legend("HHb [DC] ", "HHb [AC]", "Location", "best");
ylim([ymin ymax]);
grid on;

figure;
plot(t_trial, dHbT, "LineWidth", 1.2); hold on; plot(t_trial, DtHb); 
drawPhaseOcclusion(cycles); title("tHb  [DC/AC]"); legend("tHb [DC] ", "tHb [AC]", "Location", "best");
ylim([ymin ymax]);
grid on;
