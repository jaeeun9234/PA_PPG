...
%% envelope plotting (red/ir)
env_red = envelope(red_raw(mask), 100, 'peak');

figure;
plot(tt, env_red); hold on;
plot(tt, red_raw(mask));
grid on;
legend("red\_env", "red\_raw");
title("red\_env & red\_raw");


env_ir = envelope(ir_raw(mask), 100, 'peak');

figure;
plot(tt, env_ir); hold on;
plot(tt, ir_raw(mask));
grid on;
legend("ir\_env", "ir\_raw");
title("ir\_env & ir\_raw");
...
%% envelope plotting
dHbO2_env = envelope(dHbO2, 100, 'peak');
dHHb_env = envelope(dHHb, 100, 'peak');
dHbT_env = envelope(dHbT, 100, 'peak');

figure;
plot(tt, dHbO2_env); hold on;
plot(tt, dHHb_env);
plot(tt, dHbT_env);
grid on;
title("\DeltaHbO2, \DeltaHHb, \DeltatHb (env) [DC]");
legend("\DeltaHbO2(env)", "\DeltaHHb(env)", "\DeltatHb(env)");

%% envelope 기준 SO2
esp_den = 1e-8;
dHbT_env(abs(dHbT_env) < esp_den) = NaN;

SO2 = dHbO2_env ./dHbT_env;
SO2_s = movmean(SO2, round(2*Fs), 'omitnan');
figure;
plot(tt, SO2);
grid on;
title("\Delta HbO2\_env / \Delta tHb\_env [DC]");
...
%% HbDiff (env)
HbDiff_env = envelope(HbDiff, 100, 'peak');
figure;
plot(tt, HbDiff_env);
grid on;
title("HbDiff (env)");
...
DHbO2_env = envelope(DHbO2, 100, 'peak');
DHHb_env = envelope(DHHb, 100, 'peak');
DtHb_env = envelope(DtHb, 100, 'peak');
...
plot(tt, DHHb_env);
grid on;
title("\DeltaHbO2, \DeltaHHb \Delta tHb (env) [AC]");
legend("\DeltaHbO2(env)", "\DeltaHHb(env)");
...
Fs = 1/mean(diff(t_use1));
tu = (t_use1(1):1/Fs:t_use1(end)).';
xu = interp1(t_use1, x, tu, 'linear', 'extrap');
...
title("FFT of \DeltaHbT [AC] (0–2.5 Hz)");
...
xu = interp1(t_use, y, tu, 'linear', 'extrap');
...
grid on;
xlabel("Frequency (Hz)"); ylabel("Magnitude");
title("FFT of \DeltaHbT [DC] (0–0.7 Hz)");
...
%% 변화량 plot(envelope)
figure;
plot(tt, dHbO2_env); hold on; plot(tt, DHbO2_env); 
title("\Delta HbO2 (env) [DC/AC]"); legend("\Delta HbO2 [DC] (env)", "\Delta HbO2 [AC] (env)");
grid on;

figure;
plot(tt, dHHb_env); hold on; plot(tt, DHHb_env); 
title("\Delta HHb (env) [DC/AC]"); legend("\Delta HHb [DC] (env)", "\Delta HHb [AC] (env)");
grid on;

figure;
plot(tt, dHbT_env); hold on; plot(tt, DtHb_env); 
title("\Delta tHb (env) [DC/AC]"); legend("\Delta tHb [DC] (env)", "\Delta tHb [AC] (env)");
grid on;