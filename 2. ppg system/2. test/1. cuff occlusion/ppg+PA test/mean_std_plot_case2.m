...
%% raw / ac : hampel filter 적용
N = 20; nsigma = 2.5;
red_raw = hampel(red_raw, N, nsigma);
ir_raw = hampel(ir_raw, N, nsigma);

red_ac = hampel(red_ac, N, nsigma);
ir_ac = hampel(ir_ac, N, nsigma);
...
%% RAW, AC [bpf]
% red_raw = fillmissing(red_raw, 'linear');
% ir_raw = fillmissing(ir_raw, 'linear');
red_ac = fillmissing(red_ac, 'linear');
ir_ac = fillmissing(ir_ac, 'linear');

% red_raw_bp = bandpass(red_raw, [0.7 4], Fs);
% ir_raw_bp = bandpass(ir_raw, [0.7 4], Fs);
...
minAC_RED = median(AC_red_rms, 'omitnan') * 0.2;
minAC_IR = median(AC_ir_rms, 'omitnan') * 0.2;

bad_ac = AC_red_rms < minAC_RED | AC_ir_rms < minAC_IR;
R(bad_ac) = NaN;
...
R_s = movmean(R, round(Fs*5), 'omitnan');   % 2초 smoothing
