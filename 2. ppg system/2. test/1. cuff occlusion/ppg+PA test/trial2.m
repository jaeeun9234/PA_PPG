...
%% ===== Trial mean ± std plot: trial 2 끝나는 시점 기준으로 자르기 =====

trials = unique(trial_id);

refTrial = 2;   % trial 2 기준으로 자르기
t_end_ref = max(t_trial(trial_id == refTrial), [], 'omitnan');

t_common = linspace(0, t_end_ref, 1000);
mask2 = t_common >= 5;

fprintf("Common time axis: 0 ~ %.2f sec (based on trial %d end)\n", ...
        t_end_ref, refTrial);

%% ===== helper function처럼 반복 사용할 변수 =====
% 각 for문 안에서 t_tr <= t_end_ref 조건으로 trial1도 trial2 길이에 맞춰 자름
...
for i = 1:length(trials)
    ...
    cut_mask = t_tr <= t_end_ref;
    t_tr = t_tr(cut_mask);
    red_raw_tr = red_raw_tr(cut_mask);
    red_dc_tr  = red_dc_tr(cut_mask);
...
fill([t_common(mask2) fliplr(t_common(mask2))], ...
    [red_raw_mean(mask2)' + red_raw_std(mask2)' ...
     fliplr(red_raw_mean(mask2)' - red_raw_std(mask2)')], ...
    [0.8 0.8 1], 'EdgeColor', 'none');

fill([t_common(mask2) fliplr(t_common(mask2))], ...
    [red_dc_mean(mask2)' + red_dc_std(mask2)' ...
     fliplr(red_dc_mean(mask2)' - red_dc_std(mask2)')], ...
...
grid on;
xlabel("Time (s)");
legend("red raw ± std", "red dc ± std", "red raw mean", "red dc mean", ...
       "Location", "best");
title("RED");
...
for i = 1:length(trials)
    ...
    cut_mask = t_tr <= t_end_ref;
    t_tr = t_tr(cut_mask);
    ir_raw_tr = ir_raw_tr(cut_mask);
    ir_dc_tr  = ir_dc_tr(cut_mask);
...
fill([t_common(mask2) fliplr(t_common(mask2))], ...
    [ir_raw_mean(mask2)' + ir_raw_std(mask2)' ...
     fliplr(ir_raw_mean(mask2)' - ir_raw_std(mask2)')], ...
    [0.8 0.8 1], 'EdgeColor', 'none');

fill([t_common(mask2) fliplr(t_common(mask2))], ...
    [ir_dc_mean(mask2)' + ir_dc_std(mask2)' ...
     fliplr(ir_dc_mean(mask2)' - ir_dc_std(mask2)')], ...
    [1 0.8 0.8], 'EdgeColor', 'none');
...
grid on;
xlabel("Time (s)");
legend("ir raw ± std", "ir dc ± std", "ir raw mean", "ir dc mean", ...
       "Location", "best");
title("IR");
...
for i = 1:length(trials)
    ...
    cut_mask = t_tr <= t_end_ref;
    t_tr = t_tr(cut_mask);
    spo2_tr = spo2_tr(cut_mask);
    sto2_tr = sto2_tr(cut_mask);
...
for i = 1:length(trials)
    ...
    cut_mask = t_tr <= t_end_ref;
    t_tr = t_tr(cut_mask);
    HHb_dc_tr  = HHb_dc_tr(cut_mask);
    HbO2_dc_tr = HbO2_dc_tr(cut_mask);
    tHb_dc_tr  = tHb_dc_tr(cut_mask);
...
for i = 1:length(trials)
    ...
    cut_mask = t_tr <= t_end_ref;
    t_tr = t_tr(cut_mask);
    HHb_ac_tr  = HHb_ac_tr(cut_mask);
    HbO2_ac_tr = HbO2_ac_tr(cut_mask);
    tHb_ac_tr  = tHb_ac_tr(cut_mask);
...
for i = 1:length(trials)
    ...
    cut_mask = t_tr <= t_end_ref;
    t_tr = t_tr(cut_mask);
    HHb_r_tr  = HHb_r_tr(cut_mask);
    HbO2_r_tr = HbO2_r_tr(cut_mask);
    tHb_r_tr  = tHb_r_tr(cut_mask);
